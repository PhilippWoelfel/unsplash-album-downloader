...
def get_user_collections(username, mode):
    ...
    if user_profile['total_collections'] == 0:
        ...
        return
    collection_ids = get_collection_ids(USER_URL + user_profile['username'] + '/collections/',
                                        user_profile['total_collections'])
    photo_ids = dict()
    for cid in collection_ids:
        photo_ids = get_photo_ids(cid['url'], cid['total_photos'], mode)
        user_directory = os.getcwd() + r'/' + user_profile['name'] + '-unsplash-collections-' + mode + r'/' + cid[
            'title']
        save_photos(user_directory, photo_ids)
    ...
    for page_number in range(1, total_pages + 1):
        ...
        for i in collection_ids_list:
            collection_ids.append(
                {'id': i['id'], 'title': i['title'], 'total_photos': i['total_photos'], 'url': i['links']['photos']})
